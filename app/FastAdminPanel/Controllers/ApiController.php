...
class ApiController extends \App\Http\Controllers\Controller
{
	...
	public function getMenu()
	{
		...
		$tables = ['orders', 'careers', 'callback_contacts', 'callback_horizontal']; //name tables for count in sidebar
	...
	public function getMainpage()
	{
		...
		$alldata = [   
			'allproducts' 	=> Schema::hasTable('offers_'.Lang::get()) ? DB::table('offers_'.Lang::get())->count() : 0,
			'productsale' 	=> Schema::hasTable('orders_product') ? DB::table('orders_product')->sum('count') : 0,
			'callbackall' 	=> DB::table('careers')->count(),
			'allorders' 	=> Schema::hasTable('orders') ? DB::table('orders')->count() : 0,
			'orderstoday' 	=> Schema::hasTable('orders') ? DB::table('orders')->where('created_at','>', $today)->count() : 0,
			'ordersmonth' 	=> Schema::hasTable('orders') ? DB::table('orders')->where('created_at','>', $thismonth)->count() : 0,
		];
